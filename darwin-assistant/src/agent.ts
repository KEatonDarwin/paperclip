...
export function buildToolsBlock(): string {
  ...
  return [
    ...
    'These tools are available NATIVELY as `mcp__jarvis__<name>` — call them like any other native tool. If a tool by that name doesn\'t appear in your tool list, it\'s deferred; call `ToolSearch` with `select:mcp__jarvis__<name>` to load its schema, then call it directly. Do not conclude a listed tool is "not wired" just because the native call isn\'t visible yet — search for it first. The `<tool_call>` text block below is a FALLBACK for the rare case the native form is unavailable (e.g. this harness/adapter has no MCP config attached): output it EXACTLY in that format and STOP — the harness parses it, runs the tool, and feeds the result back as the next turn.',
    '<tool_call>',
...
export async function runClaude(
  ...
  if (toolContext && adapter.id === 'claude') {
    ...
    try {
      ...
      const config = {
        mcpServers: {
          jarvis: {
            ...
            args: [serverEntry],
            env: {
              JARVIS_TOOL_CONTEXT: JSON.stringify(toolContext),
      ...
      writeFileSync(mcpConfigPath, JSON.stringify(config), 'utf8');
...
async function runConversationTurn(
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    try {
      result = await runClaude(stdinContent, sessionId, onStreamEvent, runClaudeRuntime, signal, imageDirs, imagePaths, toolContext);
      ...
      if (sessionId && !result.text && !result.sessionId) {
        ...
        accumulatedText = '';
        sseBus.emit('sse', { type: 'stream_start', conversationId: conv.id } satisfies StreamStartEvent);
        result = await runClaude(stdinContent, null, onStreamEvent, runClaudeRuntime, signal, imageDirs, imagePaths, toolContext);
        sseBus.emit('sse', { type: 'stream_end', conversationId: conv.id } satisfies StreamEndEvent);
      }
    } catch (err) {
      sseBus.emit('sse', { type: 'stream_end', conversationId: conv.id } satisfies StreamEndEvent);
