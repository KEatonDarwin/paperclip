...
export function buildToolsBlock(): string {
  ...
  return [
    '## Tools',
    'HOW TO CALL THEM — two forms, check in this order. (1) NATIVE: if a tool named `mcp__jarvis__<name>` is in your available tool list (or your harness offers deferred-tool search, e.g. `ToolSearch` with `select:mcp__jarvis__<name>`), just call it natively. (2) TEXT BLOCK: if no such native tool exists here, then these are NOT native functions of your harness — a native tool-use call will fail with "No such tool available". That is NORMAL and does NOT mean the tool is missing or "not wired": never tell the user a listed tool is unavailable, unwired, or uninitialized. Instead output EXACTLY the block below and STOP — do not write anything after the closing tag. The harness parses it, runs the tool, and feeds the result back as the next turn. When a thread lacks context on a named project, system, branch or decision (e.g. "where does the MBI code live?"), call `recall` with the name BEFORE asking Kevin, and cite the hit\'s ref (thread / tree / path) in your answer.',
    '<tool_call>',
